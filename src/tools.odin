...
exploreMove :: proc(state: ^Game_State, move: [2][2]i8, input: string) -> Reversible_Move {
    result: Reversible_Move = {
        moveType = (state.board[move[1][0]][move[1][1]] == 0)?Move_Type.standard:Move_Type.capture,
        from = move[0],
        to = move[1],

        movedPiece = state.board[move[0][0]][move[0][1]],
        capturedPiece = state.board[move[1][0]][move[1][1]],
    ...
    if(state.board[move[0][0]][move[0][1]] == BLACK*PAWN && move[0][0] == 4 && move[0][1] != move[1][1] &&
    state.board[move[1][0]][move[1][1]] == 0) {
        result.moveType = Move_Type.enPassant
        result.capturedPiece = state.board[move[0][0]][move[1][1]]
        result.enPassantCaptureSquare = {move[0][0], move[1][1]}
        state.whitePiecesCaptured += 1
        state.board[move[0][0]][move[1][1]] = 0
    }
    if(state.board[move[0][0]][move[0][1]] == WHITE*PAWN && move[0][0] == 3 && move[0][1] != move[1][1] &&
    ...
    // Rook castling piece movement
    if(move == {{0,4},{0,2}} && state.board[0][2] == BLACK*KING) {
        result.moveType = Move_Type.castling
    ...
    result.piecePromotion = state.board[move[1][0]][move[1][1]]
...
reverseExplore :: proc(state: ^Game_State, move: Reversible_Move) {
    switch move.moveType {
        ...
        case Move_Type.capture:
            state.board[move.to[0]][move.to[1]] = move.capturedPiece
        case Move_Type.standard:
            state.board[move.to[0]][move.to[1]] = 0
    }
    state.board[move.from[0]][move.from[1]] = move.movedPiece
